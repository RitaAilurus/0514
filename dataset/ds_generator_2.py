...
from dataset.map_factory_2 import GridFactory
...
def generate_sample(h, w, min_dist, max_ds_it, obst_margin, goal_margin, location=PATH):
    feasible = False
    while not feasible:
        feasible, sample = GridFactory.make_sample(h, w, min_dist, max_ds_it, obst_margin, goal_margin)
...
class RandomGridGenerator(object):
    def __init__(self, h, w, min_dist_th, max_ds_it, obst_margin, goal_margin):
        super(RandomGridGenerator, self).__init__()
        self.h = h
        self.w = w
...
if __name__ == '__main__':
    rmg = RandomGridGenerator(100, 100, 20, 10000, 1, 0)
    # rmg.execute(20000, n_process=2)
    rmg.execute(30, n_process=2)
