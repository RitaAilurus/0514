...
def chunk_data(datapath, chunk_size=10):
    datapath = os.path.abspath(datapath)
    basename = 'data_'
    chunks = []
    chunk_count = 0
...
def custom_collate_fn(batch):
    maps = torch.stack([sample[0].grid_map for sample in batch]).unsqueeze(1)
    starts = torch.stack([sample[0].start for sample in batch])
    goals = torch.stack([sample[0].goal for sample in batch])
    paths = []
    for s in batch:
        sample = s[0]
        path = torch.zeros_like(sample.grid_map, dtype=sample.grid_map.dtype)
...
if __name__ == '__main__':
    move_data('grid_dataset', 0.8, 0.1,0.1 )
