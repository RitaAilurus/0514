...
TRAIN = 'grid_dataset/train'
VALIDATION = 'grid_dataset/validation'
EPOCHS = 100
SAVE_EVERY = 20
SHOW_EVERY = 10
...
def save_model_result(map, model_out, start, goal, path):
    ...
    sep = np.zeros((map.shape[-1], 10, 3), dtype=np.uint8)
    sep[:, :] = np.array((255, 255, 255))
...
def test(model, dataloader, loss=None, device=DEVICE, show_every=32):
...
def train(model, dataloader, loss, optimizer, epochs, device=DEVICE, save_every=30,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_dataloader is not None:
            ...
            update_loss_plot(eval_losses, train_losses)
...
def update_loss_plot(eval_losses, train_losses):
    plt.figure(1)
    plt.xlabel('Epochs')
    plt.title('training and Validation loss')
    plt.plot(np.arange(len(eval_losses)), np.array(eval_losses), color='r', label='Validation loss')
    plt.plot(np.arange(len(train_losses)), np.array(train_losses), color='b', label='Training loss')
    plt.legend()
    plt.pause(0.01)
    save_path = os.path.join(RESULTS_PATH,"loss.png")
...
def visualize_results(path):
    ...
    save_path = os.path.join(RESULTS_PATH, f"result_{len(files)}.png")
...
def main(epochs=EPOCHS, device=DEVICE):
    ...
    checkpoint_path = os.path.join(CHECKPOINT_PATH, "checkpoint_20.pt")
    if os.path.exists(checkpoint_path):
        checkpoint = Checkpoint.load_checkpoint(checkpoint_path,model)
        ...
        start_epoch = 0
    train(model, dataloader, loss, optimizer, epochs, val_dataloader=val_dataloader, save_every=SAVE_EVERY, scheduler=scheduler, checkpoint_path=CHECKPOINT_PATH,start_epoch=start_epoch)
